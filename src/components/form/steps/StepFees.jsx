          ...
          {(fee.slabs||[]).map((sl,si)=>(
            <div key={sl.id}>
              <div className="grid gap-1 mb-1 items-center" style={{gridTemplateColumns:'1fr 1fr 1fr 1.4fr 20px'}}>
                <input value={sl.from} readOnly={si>0} onChange={e=>updSlab(si,'from',e.target.value)}
                  className={`${inp} font-mono`} style={{...s6,background:si>0?'#f8fafc':'#fff'}}/>
                <input value={sl.to} onChange={e=>updSlab(si,'to',e.target.value)} placeholder="∞"
                  className={`${inp} font-mono`} style={s6}/>
                <input value={sl.rate} onChange={e=>updSlab(si,'rate',e.target.value)} placeholder="0.00"
                  className={`${inp} font-mono`} style={s6}/>
                ...
                <button type="button"
                  onClick={()=>{if((fee.slabs||[]).length>1)u('slabs',(fee.slabs||[]).filter((_,i)=>i!==si));}}
              ...
              {sl.rateType==='Others' && (
                <div className="flex items-center gap-2 mb-2 pl-1">
                  ...
                  <input value={sl.rateTypeCustom||''} onChange={e=>updSlab(si,'rateTypeCustom',e.target.value)}
                    placeholder="e.g. AI Video, Tech Pack, Image…"
                    className="flex-1 text-xs px-2 py-1 rounded-lg border focus:outline-none bg-white" style={s6}/>
                  {sl.rateTypeCustom && (
                    <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-teal-50 text-teal-700 whitespace-nowrap">
                      → {sym} per {sl.rateTypeCustom}
                    </span>
                  )}
                </div>
              )}
...
function SvcBlock({ svc, idx, onChange, onRemove, termMonths, ro, currency }) {
  ...
  return (
    <div className="border rounded-2xl mb-4 overflow-hidden" style={{borderColor:'#e2e8f0'}}>
      <div className="flex items-center justify-between px-5 py-3 bg-slate-50 border-b border-slate-200">
        <div className="flex items-center gap-3">
          ...
          {ro
            ...
            : <select value={svc.name||''} onChange={e=>onChange({...svc,name:e.target.value,subServices:[]})}
      ...
      <div className="p-5">
        {/* Sub-service multi-select — Konnect and StoreOS only */}
        {subOptions.length > 0 && (
          <div className="mb-4 p-3 rounded-xl border border-slate-200 bg-slate-50">
            ...
            {selected.length > 0 && (
              <p className="text-[10px] mt-2 text-brand-faint">
...
export default function StepFees({ form, set, ro }) {
  ...
  const addSvc  = () => set('services_fees', [...services, {id:uid(), name:'', subServices:[], fees:[newFee()]}]);
