  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(o => !o)}
        ...
        <span>⚙ Columns</span>
        <span className="text-xs px-1.5 py-0.5 rounded-full font-bold" style={{ background:T, color:'#fff' }}>{visible.length}</span>
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-1 z-30 bg-white rounded-2xl shadow-xl border border-slate-200 p-3 w-52">
          <div className="text-[10px] font-bold uppercase tracking-widest mb-2 px-1" style={{ color:'#94a3b8' }}>Toggle columns</div>
          {ALL_COLUMNS.map(col => (
            <label key={col.id}
              ...
              <input type="checkbox" checked={visible.includes(col.id)} onChange={() => toggle(col.id)} disabled={col.alwaysOn} className="accent-teal-500"/>
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4">
        ...
        <div className="mb-4">
          ...
          <textarea rows={3} value={comment} onChange={e=>setComment(e.target.value)}
            className="field-input resize-none" style={{ borderColor:'#e2e8f0' }} placeholder="Optional"/>
        </div>
        <div className="mb-4 p-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-xs">
          ⚠️ Changing to <strong>Void</strong> zeros the OF value. <strong>Churn</strong> flags partial revenue. <strong>Dropped</strong> archives the deal.
        </div>
...
export default function Repository() {
  ...
  const fyOptions = [...new Set(
    forms.filter(f => f.signed_date).map(f => getSigningQuarter(f.signed_date)?.fy).filter(Boolean)
  )].sort((a,b) => b-a);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-3">
        ...
        <select value={st} onChange={e=>setSt(e.target.value)} className="text-sm border rounded-xl px-3 py-2.5 bg-white border-slate-200">
          <option value="all">All status</option>
          {Object.entries(STATUS).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}
        </select>
        <select value={sortBy} onChange={e=>setSortBy(e.target.value)} className="text-sm border rounded-xl px-3 py-2.5 bg-white border-slate-200">
      ...
      <div className="grid grid-cols-4 gap-3 mb-3">
        {!isSales && (
          <>
            <select value={teamFilter} onChange={e=>setTeamFilter(e.target.value)} className="text-sm border rounded-xl px-3 py-2.5 bg-white border-slate-200">
              <option value="all">All teams</option>
              {SALES_TEAMS.map(t=><option key={t} value={t}>{t}</option>)}
            </select>
            <select value={repFilter} onChange={e=>setRepFilter(e.target.value)} className="text-sm border rounded-xl px-3 py-2.5 bg-white border-slate-200">
        ...
        <select value={leadCatFilter} onChange={e=>setLeadCatFilter(e.target.value)}
          ...
          {channelFilter==='Indirect'
            ...
            : <><option value="Event">Event</option><option value="Inside Sales/Pre-Sales">Inside Sales / Pre-Sales</option><option value="NA">NA</option>{channelFilter==='all'&&<option value="Partner">Partner</option>}</>
          }
        </select>
      </div>
      ...
      <div className="flex items-center gap-3 mb-3 flex-wrap">
        <span className="text-xs text-brand-faint whitespace-nowrap font-medium">Start date:</span>
        <input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} className="text-sm border rounded-xl px-3 py-2 bg-white border-slate-200"/>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <input type="date" value={signedFrom} onChange={e=>setSignedFrom(e.target.value)}
          ...
          style={signedFrom?{borderColor:T,boxShadow:`0 0 0 2px ${T}33`}:{}}/>
        <span className="text-xs text-brand-faint">to</span>
        <input type="date" value={signedTo} onChange={e=>setSignedTo(e.target.value)}
          className="text-sm border rounded-xl px-3 py-2 bg-white border-slate-200"
          style={signedTo?{borderColor:T,boxShadow:`0 0 0 2px ${T}33`}:{}}/>
        {(signedFrom || signedTo) && (
              ...
              <>
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    {show('live_date')  && <th className={thCls}>Live Date</th>}
                    ...
                    return (
                      <tr key={f.id} onClick={()=>navigate(`/form/${f.id}`)}
                          ...
                          <td className="px-4 py-3.5">
                            ...
                            {f.arr_text && (
                              <div className="text-[10px] text-brand-faint mt-0.5 leading-tight"
                                ...
                                title={f.arr_text}>{f.arr_text.split('\n')[0]}</div>
                            )}
                            {f.signed_of_link && (
                              <a href={f.signed_of_link} target="_blank" rel="noreferrer"
                                onClick={e=>e.stopPropagation()}
                                className="text-[10px] font-medium hover:underline mt-0.5 block" style={{ color:T }}>
                                📎 Signed PDF
                              </a>
                            )}
                          </td>
                        ...
                        {show('signed_date') && (
                          <td className="px-4 py-3.5 text-xs text-brand-muted whitespace-nowrap">
                            {f.signed_date
                              ? <span className="text-green-600 font-medium">✍️ {fmtShort(f.signed_date)}</span>
      ...
      {statusModal && (
        <StatusChangeModal form={statusModal} onClose={()=>setStatusModal(null)} onSave={handleStatusSave}/>
      )}
    </div>
  );
}
