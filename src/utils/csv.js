...
export const exportOFIndex = forms => {
  const hdr = ['#','OF No','QTR','FY','Customer','Brand','Services','Business Unit(s)',
    'Team','Rep','Revenue Architect','Lead Type','Sale Type','Start','End','ARR','Committed Revenue','Currency','Status'];
  const rows = forms.map((f,i) => [
    ...
    formBusinessUnits(f).join('; '), f.sales_team, f.sales_rep_name, (f.ra_email==='NA'?'NA':f.ra_name)||'', f.lead_type, f.sale_type,
