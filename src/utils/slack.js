...
function buildMessage(event, form, extra = {}) {
  ...
  // Use hash routing URL for GitHub Pages
  const url = `https://muskankatta.github.io/Order_Form/#/form/${form.id}`;

  // Tag sales rep if we have their Slack ID, else use name
  ...
  // Tag RevOps approvers on submission
  if (event === 'submitted' && form.revops_approvers?.length) {
    const tags = form.revops_approvers
      .map(email => slackMention(email) || email)
      .join(', ');
    text += `• RevOps: ${tags}\n`;
  }

  // Tag Finance approvers on revops approval
  if (event === 'revops_approved' && form.finance_approvers?.length) {
    const tags = form.finance_approvers
      .map(email => slackMention(email) || email)
      .join(', ');
    text += `• Finance: ${tags}\n`;
  }
