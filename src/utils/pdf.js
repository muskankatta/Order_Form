...
export const openPDF = form => {
  ...
  const html = '<!DOCTYPE html><html><head><meta charset="UTF-8">' +
    ...
    '@media print{button,#actionBar{display:none}h2,.sign-box h4{-webkit-print-color-adjust:exact;print-color-adjust:exact}}' +
    '</style></head><body>' +
    '<div id="of-content">' +
    '<div class="hdr">' +
  ...
  const inject = () => {
    ...
    const btn    = w.document.getElementById('combineBtn');
    const status = w.document.getElementById('pdfStatus');
      ...
      try {
        ...
        for (let i = 0; i < attachments.length; i++) {
          ...
          try {
            ...
            const aPdf  = await PDFDocument.load(bytes, { ignoreEncryption: true });
            const aPages= await merged.copyPages(aPdf, aPdf.getPageIndices());
        ...
        a.href     = url;
        a.download = 'Combined_' + ofNumber + '.pdf';
        a.click();
