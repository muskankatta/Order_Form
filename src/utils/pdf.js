...
import { entityOf } from '../constants/entities.js';
...
// ── Step-up cycles helper (mirrors formatting.js for popup window context) ────
var cyclesInDateRange = function(startDate, endDate, billingCycle) {
  if (!startDate || !endDate) return 0;
  var s = new Date(startDate);
  var e = new Date(endDate);
  var eExcl = new Date(e.getTime() + 86400000);
  var totalMonths = (eExcl.getFullYear() - s.getFullYear()) * 12 + (eExcl.getMonth() - s.getMonth());
  if (totalMonths <= 0) return 0;
  var m = { Monthly:1, Quarterly:3, 'Bi-Annually':6, Annually:12 }[billingCycle];
  if (!m) return 1;
...
export const openPDF = function(form) {
  ...
  var entitySignatoryName  = entCfg.signatory.name;
  var entitySignatoryDesig = entCfg.signatory.designation;
  var entitySignatoryLabel = entCfg.signatory.forLabel;
  var entitySalesRepLabel  = entCfg.salesRepLabel;
  var footerEntity         = entCfg.footerEntity;
  ...
  if (isYavi) {
    hdrHtml = '<div class="hdr">' +
      ...
      '</div>';
  } else if (entKey === 'fynduk') {
    hdrHtml = '<div class="hdr" style="display:flex;justify-content:space-between;align-items:flex-start">' +
      '<div style="display:flex;align-items:center;gap:14px">' +
      '<img src="' + FYND_BRANDMARK_IMG + '" style="height:52px;width:auto;display:block" alt="Fynd"/>' +
      '<div>' +
      '<div style="font-weight:700;font-size:10.5px;color:#1B2B4B">' + entCfg.header.name + '</div>' +
      '<div style="font-size:9.5px;color:#444;line-height:1.65">' + entCfg.header.lines.join('<br/>') + '</div>' +
      '</div></div>' +
  ...
  // ── Important Notes: Finance-edited or default T&C ────────────────────────
  var tcHtml;
  if (form.important_notes) {
    ...
    tcHtml =
      ...
      'a. Non-Renewal Notice \u2014 Where auto-renewal is applicable, if the Client does not wish to renew this Order Form, the Client shall provide Fynd with written notice of non-renewal at least thirty (30) days prior to the Order Form End Date. Failing such notice, the Order Form shall automatically renew in accordance with the applicable renewal terms.<br/>' +
    ...
    tcHtml =
      '<strong>1. Ownership &amp; Licensing</strong> \u2014 Shopsense Retail Technologies Limited (\u201cFynd\u201d) is the owner of the Software/Platform. Shopsense Retail Technologies (UK) Limited (\u201cCompany\u201d, \u201cwe,\u201d or \u201cus\u201d) is the contracting party under this Order Form and has been granted a licence by Fynd to market, distribute, and grant access the Service(s) in the United Kingdom pursuant to a separate intra-group licence/reseller agreement between Fynd and the Company. The Company provides the Service(s) to the Client under this Order Form on that basis.<br/>' +
      '<strong>2. Agreement Scope</strong> \u2014 This Order Form shall be read together with schedules, annexures, SOP(s), SoW(s), and/or any written documents executed between the Parties, read along with the online terms and policy documents of the Company with respect to the Service(s) being availed by the Client, and shall constitute the entire understanding and agreement between the Parties and replaces all prior understandings, negotiations, discussions, writings and agreements with respect to the subject matter hereof.<br/>' +
      '<strong>3. Term</strong> \u2014 The Service Period and all applicable Renewal Tenures are collectively referred to herein as the \u201cOrder Form Term\u201d. This Order Form is effective on the date the Service Period commences until the end of the Order Form Term. Renewal will be applicable on then-current list price.<br/>' +
      'a. Non-Renewal Notice \u2014 Where auto-renewal is applicable, if the Client does not wish to renew this Order Form, the Client shall provide the Company with written notice of non-renewal at least thirty (30) days prior to the Order Form End Date. Failing such notice, the Order Form shall automatically renew in accordance with the applicable renewal terms.<br/>' +
      '<strong>4. Fees</strong> \u2014 Client will be charged fees as set forth in this Order Form upon its execution and in accordance with the applicable Billing Frequency (as defined above) thereafter. All fees (commercial value) that Client is charged, including the fees set forth in this Order Form, will be quoted in Pounds Sterling (GBP) and are exclusive of any other applicable taxes, duties, or levies. If Client terminates this Order Form prior to the expiration of the Initial Term or then-current Renewal Term (except to the extent such termination is due to Company\u2019s failure to cure a material breach in accordance with the Agreement (as defined in the Terms of Service)), then Client is responsible for paying the fees set forth in this Order Form for the remaining portion of the Initial Term or then-current Renewal Term upon termination. All fees except one-time fees will be applicable for a minimum increment of 8% on then-current list price (as shared by the Company to Client) upon Renewal Term.<br/>' +
      '<strong>5. Governing Law &amp; Jurisdiction</strong> \u2014 This Order Form, and any dispute or claim arising out of or in connection with it or its subject matter (including non-contractual disputes or claims), shall be governed by and construed in accordance with the laws of England and Wales. The Parties irrevocably agree that the courts of England and Wales shall have exclusive jurisdiction to settle any such dispute or claim.<br/>' +
    ...
    tcHtml =
      ...
      'e. Delayed Payment Interest \u2014 If the Client fails to pay any amount payable under this Order Form by the applicable due date, the overdue amount shall carry an interest at the rate of 1.5% (one point five per cent) per month, calculated on a pro-rata basis from the due date until the date of actual payment in full. Such interest shall accrue automatically, without the requirement of any further notice, and shall be without prejudice to Fynd\u2019s other rights and remedies under the Agreement or applicable law.<br/>' +
  ...
  var feeRow = function(fee) {
    ...
    if (fee.isLogistics) {
      ...
      cv = fee.stepUpValues.map(function(sv, pi) {
        var rate = parseFloat(sv.rate != null ? sv.rate : sv.value) || 0;
        var cycles = (sv.startDate && sv.endDate && sv.billingCycle)
          ? cyclesInDateRange(sv.startDate, sv.endDate, sv.billingCycle) : null;
        var label = (sv.startDate && sv.endDate)
